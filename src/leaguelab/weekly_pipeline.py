...
from leaguelab.upcoming_matchups import build_upcoming_matchups
...
def run_pipeline(
    ...
    if skip_normalize:
        ...
        if normalize is None:
            ...
            print(
                "      Normalizer module not found; "
                "using existing normalized files."
            )
        else:
            normalize(season)
            print("      OK")

    # Step 3: build weekly analytics.
    print("")
    ...
    # Step 4: build all newsletter-specific supporting data, then render HTML.
    print("")
    print("[4/4] Newsletter HTML")

    challenge_data = build_challenge_newsletter_data(
        season=season,
        week=week,
    )
    ...
    upcoming_data = build_upcoming_matchups(
        season=season,
        week=week,
        analytics_result=result,
    )
    ...
    if week >= 14:
        postseason_data = build_postseason_newsletter_data(
            season=season,
            week=week,
            analytics_result=result,
        )
